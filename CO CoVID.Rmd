...
names(COdataRWJ2020raceFULL) %<>% 
  ...
  gsub(".Native", "_Native", ., fixed=T) %>% 
  gsub("_Hispanic_White", ".Hispanic_White", ., fixed=T) %>% 
  
  # gsub(".95LCI", "_95LCI", ., fixed=T) %>% 
  # gsub(".95HCI", "_95HCI", ., fixed=T) %>% 
  # gsub(".Z.Score", "_Z.Score", ., fixed=T) %>% 
  gsub("AIAN.American.Indian.Alaska_Native", "AIAN", ., fixed=T) %>% 
  ...
  gsub("Rate.95LCI", "Rate_ALL.95LCI", ., fixed=T) %>%
  gsub("Rate.95HCI", "Rate_ALL.95HCI", ., fixed=T) %>% 
  gsub("Rate.Z.Score", "Rate_ALL.Z.Score", ., fixed=T) %>% 
  gsub("ated.Z.Score", "ated_ALL.Z.Score", ., fixed=T) %>% 
  gsub("Poverty.95LCI", "Poverty_ALL.95LCI", ., fixed=T) %>%
  gsub("Poverty.95HCI", "Poverty_ALL.95HCI", ., fixed=T) %>% 
  gsub("Poverty.Z.Score", "Poverty_ALL.Z.Score", ., fixed=T) %>% 
  gsub("Alone.95LCI", "Alone_ALL.95LCI", ., fixed=T) %>%
  gsub("Alone.95HCI", "Alone_ALL.95HCI", ., fixed=T) %>% 
  gsub("Alone.Z.Score", "Alone_ALL.Z.Score", ., fixed=T) %>% 
  gsub("Income.95LCI", "Income_ALL.95LCI", ., fixed=T) %>%
  gsub("Income.95HCI", "Income_ALL.95HCI", ., fixed=T) %>% 
  gsub("Income.Z.Score", "Income_ALL.Z.Score", ., fixed=T)

for (i in c(2,21,40,47,54,64,83,102)) {
  colnames(COdataRWJ2020raceFULL)[i] = paste(colnames(COdataRWJ2020raceFULL)[i], "ALL", sep="_") }
...
COdataRWJ2020race_no_num = 
  COdataRWJ2020raceFULL %>%   select(., -contains("Num."))
#names(COdataRWJ2020race_no_num)
#lapply(COdataRWJ2020raceFULL, class)

COdataRWJ2020racePivot_und = COdataRWJ2020race_no_num %>%
  pivot_longer(
   cols = contains("_"), 
...
COdataRWJ2020racePivot_und$Category %<>%    
...
COdataRWJ2020raceP2 = COdataRWJ2020racePivot_und %>% separate(., Category, c("Category", "Demographic"), "_", remove = T)

COdataRWJ2020raceP3 = COdataRWJ2020raceP2 %>%
  pivot_wider(
   names_from = Test.Type, 
    values_from = Value)
#write.csv(COdataRWJ2020raceP3, "CO_Demographic_Race_Data_by_County.csv", row.names = F)

unique(COVID19_PER$COUNTY)
CO_Race_Measures_COVID19 = left_join(COdataRWJ2020raceP3, COVID19_PER, by="COUNTY")
...
names(Health_Outcomes_Master) %<>%  
  ...
  gsub("Influenza.", "Influenza.Hospitalizations.", ., fixed=T) %>% 
...
CO_COUNTY_COVID_ALL = CO_COUNTY_COVID_ALL %>% arrange(., COUNTY)
names(CO_COUNTY_COVID_ALL) %<>% gsub("Perc.Vaccinated", "Perc.Flu.Vaccinated", ., fixed=T) 

write.csv(CO_COUNTY_COVID_ALL, "CO_COUNTY_COVID_ALL.csv", row.names = F) 

CO_COUNTY_PER = CO_COUNTY_ALL %>% select(., COUNTY, contains("Perc", ignore.case = T), contains("Rate", ignore.case = T), LE.20102015, LE.STATE, -contains("95"), -contains("Z_score", ignore.case = T))
