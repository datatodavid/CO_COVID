...
Zscores2020RWJ = read.csv("2020 County Health Rankings Colorado Data Life Expectancy By County.csv", stringsAsFactors = F, header = T)
names(Zscores2020RWJ) = gsub("Socio.Economic..Factors.Rank", "Socio.Economic.Factors.Rank", names(Zscores2020RWJ))
names(Zscores2020RWJ) = gsub("County", "COUNTY", names(Zscores2020RWJ))
Zscores2020RWJ = Zscores2020RWJ %>% select(., -ï..FIPS, -X, -State) %>% mutate_at(., "COUNTY", toupper)
# unique(colnames(Zscores2020RWJ))
# unique(Zscores2020RWJ$COUNTY)
...
library(magrittr)
COdataRWJ2020$Presence.of.Water.Violation %<>%
  gsub("No", 0, .) %>% 
  gsub("Yes", 1, .)
unique(COdataRWJ2020$Presence.of.Water.Violation)
COdataRWJ2020 = COdataRWJ2020 %>% mutate_if(., !str_detect(names(.), "COUNTY"), as.numeric)
warnings()
# ZscoreClean = function(x) {
#   gsub("Z.Score.*", paste0(COdataRWJ2020names[i-1], ".Z.Score"), x)
# }
# COdataRWJ2020names = names(COdataRWJ2020)
# ZscoreClean(COdataRWJ2020names)
# ZscoreClean(names(COdataRWJ2020))
...
COdataRWJ2020race = COdataRWJ2020simp %>% select(., COUNTY, contains("YPLL"), contains("Teen."), contains("Precentable.H"), contains("Vaccinated"), contains("in.Poverty"), contains("Injury.Death"), contains("Drive.Alone"), contains("Household.Income"), contains("AIAN"), contains("Asian"), contains("Black"), contains("Hispanic"), contains("White"), contains("Native"))

# 
# #MetricsCO = unique(colnames(COdataRWJ2020))
# COdataRWJ2020 = COdataRWJ2020[-1,]
# COdataRWJ2020 = COdataRWJ2020[,-1]
# COdataRWJ2020 = COdataRWJ2020[,-1]
...
names(Health_Outcomes_Master) = gsub("COUNTY_NAME", "COUNTY", names(Health_Outcomes_Master))

#unique(colnames(Health_Outcomes_Master))

CO_COUNTY = left_join(CO_COUNTY, Health_Outcomes_Master, by = ("COUNTY"))


#Non-Hospitalization Health Rates
...
#unique(colnames(Diabetes_Rates))
#head(Diabetes_Rates)
...
#unique(colnames(Heart_Disease_Rates))
#head(Heart_Disease_Rates)
####
#### MJ Rates #####
...
# unique(colnames(Delayed_Care_Rates))
...
# Delayed_Care_Rates = Delayed_Care_Rates %>% 
#   select(., -Delayed_Care_County_Rate)
# head(Delayed_Care_Rates)
# lapply(Delayed_Care_Rates, class)
####
#### No Reg Care Rates ####
...
# unique(colnames(No_Reg_Care_Rates))
...
# No_Reg_Care_Rates = No_Reg_Care_Rates %>% 
#   select(., -No_Reg_Care_County_Rate)
#head(No_Reg_Care_Rates)
####
...
Life_Expectancy2015 =  read.csv("Colorado_Life_Expectancy_by_Census_Tract_Published_by_NAPHSIS-USALEEP_(2010-2015).csv", stringsAsFactors = F, header=T)
    # For Census, Data, need to aggregate values by County - 
    # This should really be an inner_join and then sum(pop.tract*LE_20102015tract)/pop.county
...
# unique(colnames(Life_Expectancy2015))
# unique(Life_Expectancy2015)
# head(Disability)      
#     # ?select_if
#     # negate(grepl("Percent", colnames, fixed=T)))))
#     # vars(-contains("Percent")), funs(x = x)) %>% 
# #lambda function  
#       # mutate(., County_Population_Total = sum(Population_Total)) %>% 
#     unique(Disability)


# Nursing homes / ind living facilities filter 
# How much /easy access to medical care availabnle
...
# lapply(Health_Facilities, class)
# unique(Health_Facilities$OPERATING_STATUS)
# Health_Facilities$CERTIFIED_AVAILABLE_BEDS = as.numeric(Health_Facilities$CERTIFIED_AVAILABLE_BEDS)
# Health_Facilities$CERTIFIED_TOTAL_BEDS = as.numeric(Health_Facilities$CERTIFIED_TOTAL_BEDS)
...
# head(Health_Facilities_Sum)
# head(Health_Outcomes_Master)
# head(Diabetes_Outcomes)
...
, by="COUNTY"), by="COUNTY"), by="COUNTY"), by="COUNTY"), by="COUNTY")
#unique(colnames(Health_Facilities))

# colnames(CO_COUNTY)
#### ADDING NON-HOSPITALIZATION HEALTH ####

CO_COUNTY = left_join(CO_COUNTY, Non_Hospitalization_Health, by = "COUNTY")
CO_COUNTY = CO_COUNTY %>% 
  mutate(., Per_100000_Total_Dis_Providers = 100000*Total_Dis_Providers/Population_Total,
Per_100000_Available_Beds = 100000*Available_Beds/Population_Total,
Per_100000_Total_Beds = 100000*Total_Beds/Population_Total,
Per_100000_Assisted_Living_Facs = 100000*Assisted_Living_Facs/Population_Total,
Per_100000_Total_Health_Facilities = 100000*Total_Health_Facilities/Population_Total)


CO_COUNTY_PER = CO_COUNTY %>% select(., COUNTY, contains("Per", ignore.case = T), contains("Rate", ignore.case = T), LE_20102015, LE_STATE, -contains("95"), -contains("Z_score", ignore.case = T))



DFnoCOUNTY = CO_ALL_PER %>% select(., -COUNTY)
##### COVID ######

COVID19County = read.csv("CDPHE_COVID19_County-Level_Open_Data_Repository_05_25.csv", stringsAsFactors = F, header=T)
#library(tidyr)
#COVIDtest = COVID19County %>% 
#  group_by(Metric) %>% summarise(n())
#?replace_na
...
#COVID19County = COVID19County %>% select(., -Rate, -Desc_, -ï..OBJECTID, -LABEL, -FULL_, -FIPS)
#COVIDtest
...
COVID19Positive = COVID19Positive %>% 
 ...
 COVID_County_Pos_Case_Rate_Per_100000 = County_Rate_Per_100_000, COVID_County_Total_Deaths = County_Deaths, State_Deaths, State_Number_Hospitalizations, State_Number_Tested) %>% 
...
COVID_Max_Testing_Rates_Per_100000 = max(Testing.Rates.Per.100000, na.rm = TRUE), COVID_Mean_Case_Rates_Per_100000 = mean(Case.Rates.Per.100000, na.rm = TRUE), COVID_Mean_Cases_per_Day = COVID_Cases_May25/COVID_Num_Days_Since_First_Case, COVID_Max_Case_Rates_Per_100000 = max(Case.Rates.Per.100000, na.rm = TRUE),
COVID_Perc_Days_with_Cases = COVID_Num_Days_Since_First_Case/(as.numeric(as.Date("2020-05-17")) - as.numeric(as.Date("2020-03-17"))))
...
which(COVID19CountyANALYSIS$COVID_Mean_Case_Rates_Per_100000<100)
 
lapply(COVID19CountyANALYSIS, class)
#unique(colnames(COVID19CountyANALYSIS))
COVID19_PER = COVID19CountyANALYSIS %>% select(., COUNTY, COVID_Max_Testing_Rates_Per_100000, COVID_County_Pos_Case_Rate_Per_100000, COVID_County_Death_Rate_Per_100000, COVID_Perc_Days_with_Cases) %>% 
  mutate(.,
COVID_Death_Rate_if_Pos_Case = COVID_County_Death_Rate_Per_100000/COVID_County_Pos_Case_Rate_Per_100000)
...
# Uninsured.1_Perc.Uninsured # 
### doesn't affect case rate, but does affect death rate! ###
ggplot(CO_ALL_PER, aes(x=Uninsured.1_Perc.Uninsured, y=COVID_County_Pos_Case_Rate_Per_100000)) + geom_point(aes(alpha=COVID_County_Death_Rate_Per_100000)) 


# Flu.vaccinations #
### doesn't affect case rate, but does affect death rate! ###
ggplot(CO_ALL_PER, aes(x=Flu.vaccinations_Perc.Vaccinated, y=COVID_County_Pos_Case_Rate_Per_100000)) + geom_point(aes(alpha=COVID_County_Death_Rate_Per_100000)) 

# Demographics.2_Perc.65.and.over #                  NADA
ggplot(CO_ALL_PER, aes(x=Demographics.2_Perc.65.and.over, y=COVID_County_Pos_Case_Rate_Per_100000)) + geom_point(aes(alpha=COVID_County_Death_Rate_Per_100000)) 

# Demographics.4_Perc.Black #                  NADA
ggplot(CO_ALL_PER, aes(x=Demographics.4_Perc.Black
, y=COVID_County_Pos_Case_Rate_Per_100000)) + geom_point(aes(alpha=COVID_County_Death_Rate_Per_100000)) 
...
# COVID19CountyBLOWNOUT = left_join(COVID19County, COVID19CountyANALYSIS, by="COUNTY")
# COVID19CountyJOINED
# COVID19CountyANALYSIS
# ALLDATA = full_join(COVID19CountyANALYSIS, 
#                     Non_Hospitalization_Health, by=COUNTY)
# # COVID19County = COVID19County %>% select(., -starts_with("Caution"), 
#                                          -starts_with("In"), -starts_with("Rates"), -starts_with("County rates"), 
#                                          -starts_with("Some"))
# COVID19County = COVID19County %>% group_by(COUNTY, Date)
unique(colnames(COVID19CountyANALYSIS))
lapply(COVID19CountyANALYSIS, class)
head(COVID19CountyANALYSIS$COVID_Avg_Cases_per_Day)


#### COMBINING EVERYTHING ####
CO_COUNTY_COVID = inner_join(COVID19CountyANALYSIS, CO_COUNTY, by="COUNTY")
CO_COUNTY_COVID = CO_COUNTY_COVID %>% mutate(., COVID_Death_Rate_per_100000 = 100000*COVID_County_Total_Deaths/Population_Total,
COVID_Mean_Daily_Case_Rate_per_100000 = 100000*COVID_Avg_Cases_per_Day/Population_Total)
write.csv(CO_COUNTY_COVID, "CO_COUNTY_COVID.csv", row.names = F)

CO_COUNTY_COVID_ALL = full_join(COVID19CountyANALYSIS, CO_COUNTY, by="COUNTY")
CO_COUNTY_COVID_ALL = CO_COUNTY_COVID_ALL %>% mutate(., COVID_Death_Rate_per_100000 = 100000*COVID_County_Total_Deaths/Population_Total, COVID_Mean_Daily_Case_Rate_per_100000 = 100000*COVID_Avg_Cases_per_Day/Population_Total)
